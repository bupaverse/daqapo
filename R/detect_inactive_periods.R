...
#' @param only_consider_start_activity When specified, an inactive period only occurs when the time between two consecutive arrivals exceeds the specified threshold (arrival is proxied by the timestamp of the activity/activities specified in this argument)
...
#' @export
inactive_periods <- function(activity_log, threshold_in_minutes, timestamp = "both", only_consider_start_activity = NA, details = TRUE, filter_condition = NULL){
  ...
  if(!is.na(only_consider_start_activity)){
    if(timestamp %in% c("both", "start")){
      activity_log <- as.data.frame(activity_log %>% group_by(case_id) %>%
                                      filter(activity %in% only_consider_start_activity) %>% filter(start == min(start)))
    } else{
      activity_log <- as.data.frame(activity_log %>% group_by(case_id) %>%
                                      filter(activity %in% only_consider_start_activity) %>% filter(complete == min(complete)))
    }
  }
