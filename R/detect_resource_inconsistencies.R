...
#' @param detect_only Boolean indiciating what results to return: if TRUE, immediatly return the detection of inconsistencies. If FALSE, return a new log with resolved issues if the users requests this correction
#' @return an overview containing the activity executions that do not have matching resources for their start and complete events OR the new event log with all resource inconsistencies corrected.
#' @export
