...
events_to_activities.eventlog <- function(eventlog) {
  ...
  class(activitylog) <- c("activitylog",class(eventlog) %>% subset(. != "eventlog"))
  attr(activitylog, "case_id") <- case_id(eventlog)
  attr(activitylog, "activity_id") <- activity_id(eventlog)
  attr(activitylog, "activity_instance_id") <- activity_instance_id(eventlog)
  attr(activitylog, "resource_id") <- resource_id(eventlog)
  attr(activitylog, "timestamps") <- lifecycle_labels(eventlog) %in% c("start", "complete")
